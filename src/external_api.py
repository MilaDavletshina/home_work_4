...
def get_transaction_amount(transaction) -> float:
    ...
    currency = transaction.get('currency')                       # отправляем запрос для получения инфы о валюте
    if currency == 'RUB':
        return float(amount)
    else:
        exchange_rate = external_api.get_currency_rate(currency) # получаем курс валюты из функции currency_rate
        if exchange_rate:
